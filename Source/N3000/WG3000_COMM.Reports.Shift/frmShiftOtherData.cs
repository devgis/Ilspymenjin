...
{
	public class frmShiftOtherData : frmN3000
	{
		public class ToolStripDateTime : ToolStripControlHost
		{
			...
			public ToolStripDateTime() : base(frmShiftOtherData.ToolStripDateTime.dtp = new DateTimePicker())
			{
			}
		...
		private void btnQuery_Click(object sender, EventArgs e)
		{
			...
			if (wgAppConfig.IsAccessDB)
			{
				text = " SELECT t_d_ShiftData.f_RecID, t_b_Group.f_GroupName, ";
				text += "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ";
				text += " t_b_Consumer.f_ConsumerName AS f_ConsumerName, ";
				text += " t_d_ShiftData.f_DateYM, ";
				for (int i = 1; i < 31; i++)
				{
					text += string.Format(" CSTR(t_d_ShiftData.f_ShiftID_{0:d2}) as f_ShiftID_{0:d2}, ", i);
				}
				text += "CSTR(t_d_ShiftData.f_ShiftID_31) as f_ShiftID_31 ";
			...
			else
			{
				...
				for (int j = 1; j < 31; j++)
				{
					text += string.Format(" CONVERT(nvarchar(3),  t_d_ShiftData.f_ShiftID_{0:d2}) as f_ShiftID_{0:d2}, ", j);
				}
				text += "CONVERT(nvarchar(3),  t_d_ShiftData.f_ShiftID_31) as f_ShiftID_31 ";
				text += " ,t_b_Consumer.f_ConsumerID  ";
			}
			string sqlFindNormal = wgAppConfig.getSqlFindNormal(text, "t_d_ShiftData", this.getSqlOfDateTime("t_d_ShiftData.f_DateYM"), groupMinNO, groupIDOfMinNO, groupMaxNO, findName, findCard, findConsumerID);
							...
							if (dfrmShiftEdit.ShowDialog() == DialogResult.OK)
							{
								...
								if (wgAppConfig.IsAccessDB)
								{
									using (comShift_Acc comShift_Acc = new comShift_Acc())
									{
										...
										long num = (long)comShift_Acc.shift_arrange_update(Convert.ToInt32(dataGridViewRow.Cells["f_ConsumerID"].Value), dateShift, shiftid);
										if (num == 0L)
										{
											if (shiftid == 0)
											{
								...
								using (comShift comShift = new comShift())
								{
									...
									long num2 = (long)comShift.shift_arrange_update(Convert.ToInt32(dataGridViewRow.Cells["f_ConsumerID"].Value), dateShift2, shiftid);
									if (num2 == 0L)
									{
										if (shiftid == 0)
										{
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(frmShiftOtherData));
