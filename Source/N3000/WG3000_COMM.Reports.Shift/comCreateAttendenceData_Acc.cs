...
namespace WG3000_COMM.Reports.Shift
{
	public class comCreateAttendenceData_Acc : Component
	{
		...
		private comCreateAttendenceData_Acc.CreateCompleteEventHandler CreateCompleteEvent;

		private comCreateAttendenceData_Acc.DealingNumEventHandler DealingNumEvent;
		...
		public event comCreateAttendenceData_Acc.CreateCompleteEventHandler CreateComplete
		{
			add
			{
				this.CreateCompleteEvent = (comCreateAttendenceData_Acc.CreateCompleteEventHandler)Delegate.Combine(this.CreateCompleteEvent, value);
			}
			remove
			{
				this.CreateCompleteEvent = (comCreateAttendenceData_Acc.CreateCompleteEventHandler)Delegate.Remove(this.CreateCompleteEvent, value);
			}
		}
		...
		public event comCreateAttendenceData_Acc.DealingNumEventHandler DealingNum
		{
			add
			{
				this.DealingNumEvent = (comCreateAttendenceData_Acc.DealingNumEventHandler)Delegate.Combine(this.DealingNumEvent, value);
			}
			remove
			{
				this.DealingNumEvent = (comCreateAttendenceData_Acc.DealingNumEventHandler)Delegate.Remove(this.DealingNumEvent, value);
			}
		}
			...
			try
			{
				...
				while (oleDbDataReader.Read())
				{
					...
					while (dateTime3 <= DateTime.Parse(Strings.Format(this.endDateTime.AddDays((double)this.normalDay), "yyyy-MM-dd " + this.strAllowOffdutyTime)))
					{
						...
						text += " ) ";
						using (OleDbCommand oleDbCommand = new OleDbCommand(text, this.cn))
						{
					...
					text += " )";
					using (OleDbCommand oleDbCommand2 = new OleDbCommand(text, this.cn))
					{
						if (this.cn.State == ConnectionState.Closed)
						{
							this.cn.Open();
