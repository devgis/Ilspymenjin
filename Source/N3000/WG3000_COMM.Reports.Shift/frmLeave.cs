...
{
	public class frmLeave : frmN3000
	{
		public class ToolStripDateTime : ToolStripControlHost
		{
			...
			public ToolStripDateTime() : base(frmLeave.ToolStripDateTime.dtp = new DateTimePicker())
			{
			}
		...
		private void btnQuery_Click(object sender, EventArgs e)
		{
			...
			string text = " SELECT t_d_Leave.f_NO, t_b_Group.f_GroupName, ";
			text += "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ";
			text += " t_b_Consumer.f_ConsumerName AS f_ConsumerName, ";
			text += " CONVERT(DateTime , [f_Value]) AS f_from, ";
			text += " [f_Value1] AS f_from1, ";
			text += " CONVERT(DateTime , [f_Value2]) AS f_to, ";
			text += " [f_Value3] AS f_to1, ";
			text += " t_d_Leave.[f_HolidayType], ";
			text += " t_d_Leave.f_Notes, ";
			text += " t_b_Consumer.f_ConsumerID  ";
			string sqlFindNormal = wgAppConfig.getSqlFindNormal(text, "t_d_Leave", this.getSqlOfDateTime("t_d_Leave.f_DateYM"), groupMinNO, groupIDOfMinNO, groupMaxNO, findName, findCard, findConsumerID);
		...
		private void btnQuery_Click_Acc(object sender, EventArgs e)
		{
			...
			text += "       t_b_Consumer.f_ConsumerNO AS f_ConsumerNO, ";
			text += " t_b_Consumer.f_ConsumerName AS f_ConsumerName, ";
			text += " CDate([f_Value]) AS f_from, ";
			text += " [f_Value1] AS f_from1, ";
			text += " CDate([f_Value2]) AS f_to, ";
			text += " [f_Value3] AS f_to1, ";
			text += " t_d_Leave.[f_HolidayType], ";
			text += " t_d_Leave.f_Notes, ";
			text += " t_b_Consumer.f_ConsumerID  ";
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			{
				int index = this.dgvMain.SelectedRows[0].Index;
				string strSql = " DELETE FROM t_d_Leave WHERE [f_NO]= " + this.dgvMain[0, index].Value.ToString();
			...
			else
			{
				...
				{
					string strSql = " DELETE FROM t_d_Leave WHERE [f_NO]= " + dataGridViewRow.Cells[0].Value.ToString();
					wgAppConfig.runUpdateSql(strSql);
				}
			}
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(frmLeave));
			...
			this.btnTypeSetup = new ToolStripButton();
			...
			this.f_HolidayType = new DataGridViewTextBoxColumn();
			...
			this.btnTypeSetup.ForeColor = Color.White;
			this.btnTypeSetup.Image = Resources.pTools_TypeSetup;
			componentResourceManager.ApplyResources(this.btnTypeSetup, "btnTypeSetup");
			this.btnTypeSetup.Name = "btnTypeSetup";
			this.btnTypeSetup.Click += new EventHandler(this.btnTypeSetup_Click);
			...
			base.Load += new EventHandler(this.frmLeave_Load);
