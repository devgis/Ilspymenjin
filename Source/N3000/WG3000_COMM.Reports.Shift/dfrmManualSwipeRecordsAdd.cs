...
{
	public class dfrmManualSwipeRecordsAdd : frmN3000
	{
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(dfrmManualSwipeRecordsAdd));
			...
			this.dateEndHMS2.Value = new DateTime(2010, 1, 1, 8, 0, 0, 0);
			...
			base.Load += new EventHandler(this.dfrmManualSwipeRecordsAdd_Load);
			...
			using (SqlConnection sqlConnection = new SqlConnection(wgAppConfig.dbConString))
			{
				using (SqlCommand sqlCommand = new SqlCommand(text, sqlConnection))
				{
					...
					while (i < this.dgvSelectedUsers.Rows.Count)
					{
						...
						text = text + ", " + wgTools.PrepareStr(this.dtpStartDate.Value.ToString("yyyy-MM-dd") + this.dateEndHMS2.Value.ToString(" HH:mm:ss"), true, "yyyy-MM-dd HH:mm:ss") + " AS [f_ReadDate] ";
						text = text + ", " + wgTools.PrepareStr(this.txtf_Notes.Text) + " AS [f_Note] ";
