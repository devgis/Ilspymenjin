							...
							else
							{
								...
								if (oleDbDataReader.Read())
								{
									for (int j = 1; j <= Convert.ToInt32(oleDbDataReader["f_ReadTimes"]); j++)
									{
										...
										dataRow["f_PlanTime"] = Convert.ToDateTime(Strings.Format(dateTime2.AddDays((double)(Convert.ToInt32(Strings.Format(oleDbDataReader[j + 2], "dd")) - 1)), "yyyy-MM-dd") + Strings.Format(oleDbDataReader[j + 2], " HH:mm:ss"));
