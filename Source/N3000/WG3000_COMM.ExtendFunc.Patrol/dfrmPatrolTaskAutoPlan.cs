...
{
	public class dfrmPatrolTaskAutoPlan : frmN3000
	{
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(dfrmPatrolTaskAutoPlan));
		...
		private void _dataTableLoad_Acc()
		{
			...
			try
			{
				if (wgAppConfig.IsAccessDB)
				{
					selectCommand = new OleDbCommand("SELECT [f_RouteID] & '-' & [f_RouteName] as f_RouteFullName, [f_RouteID] from t_d_PatrolRouteList order by f_RouteID ASC", oleDbConnection);
				}
				else
				{
					selectCommand = new OleDbCommand("SELECT CONVERT(nvarchar(50),[f_RouteID]) + case when [f_RouteName] IS NULL Then '' ELSE   '-' + [f_RouteName] end  as f_RouteFullName, [f_RouteID] from t_d_PatrolRouteList order by f_RouteID ASC", oleDbConnection);
				}
				try
				{
